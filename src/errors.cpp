...
int main() {
    ...
    // tc::error_code err;
    // std::cout << err.get_value() << std::endl;
    // std::cout << err.get_message().cstr() << std::endl;

    tc::error_code err(ERROR_INVALID_PARAMETER, tc::system_category());
    tc::error_condition cond(EINVAL, tc::generic_category());
    std::cout << std::boolalpha << (err == cond) << std::endl;
}
