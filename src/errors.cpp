...
//ToDo impl resize alghoritm

int resize(const unsigned char* in, int w_in, int h_in, unsigned char* out, int w_out, int h_out, int channels, tca::allocator* alloc = tca::get_default_allocator()) {
    
    float wscale = static_cast<float>(w_out) / static_cast<float>(w_in);
    float hscale = static_cast<float>(h_out) / static_cast<float>(h_in);
    
    tc::array<float> tmp (w_out * h_in * channels, alloc);

    tc::array<contribution> contrib_x(w_out);
    tc::array<contribution> contrib_y(h_out);

    { //contrib x fill array
        namespace math = tc::math;
        float rad = (wscale < 1.0f) ? (wscale / 2.0f) : 2.0f;
        for (int x = 0; x < w_out; ++x)
        {
            float center = (static_cast<float>(x) + 0.5f) / wscale;
            int start    = math::clamp(static_cast<int>(math::floor(center - rad)), 0, w_in - 1);
            int end      = math::clamp(static_cast<int>(math::ceil (center + rad)), 0, w_in - 1);
            int count    = end - start + 1;
            contribution& contr = contrib_x[x];
            contr.end     = end;
            contr.start   = start;
            contr.weights = tc::array<float>(count);
...
int main() {
    try {
        tc::image img = tc::imageio::load_image(tc::file("build/stone.png"));
        std::cout << img.to_string() << std::endl;
        {      
            tc::image resized(img.get_width() / 2, img.get_height() / 2, img.get_channels());                
            resize(
        ...
        {
            tc::image resized = img.resize(img.get_width() / 2, img.get_height() / 2);
